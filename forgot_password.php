	...
	function sendMail($email, $reset_token)
	{
		...
		try 
		{
		    ...
		    $mail->Body    = "We got a request from you to reset your password...!! <br>
		    	...
		    	<a href='http://localhost/User_Membership_Functionality/update_pass.php?email=$email&reset_token=$reset_token'>Reset Password</a>";
	...
	if(isset($_POST['send_reset_link']))
	{
		...
		if($result)
		{
			if(mysqli_num_rows($result)==1)
			{
				...
				$date=date("y-m-d");
				$query="UPDATE `registered_users` SET `resert_token`='$reset_token',`reset_token_expire`='$date' WHERE `email`='$_POST[email]'";
